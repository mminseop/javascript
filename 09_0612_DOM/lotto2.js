...
function generateNumbers() {
  ...
  */
  ...
  for (let i = 0; lottoNumbers.length < 6; i++) {
    const randomNum = Math.trunc(Math.random() * 45) + 1;
...
function displayNumbers(numbers, winningNumbers = []) {
  ...
  */
  ...
  numWrap.classList.add('number-set');
  ...
  numbers.forEach((x) => {
    ...
    if (winningNumbers.includes(x)) {
      numElem.classList.add('match');
    }
    numElem.textContent = x;
    numWrap.appendChild(numElem);
  });
